...
import { api, type R2Entry } from "@/lib/web-api";
import { formatSize } from "@/lib/web-format";
...
function BrowserHome() {
  ...
  const listingStats = useMemo(() => {
    ...
    for (const entry of visibleEntries) {
      if (entry.type === "folder") folders += 1;
      else {
        files += 1;
        bytes += entry.size ?? 0;
      }
  ...
  async function runSearch(query: string) {
    ...
    try {
      ...
      if (reqId === searchRequestId.current) setSearching(false);
    }
  }

  const refresh = useCallback(
    async (credentialId: string, p: string, opts?: { soft?: boolean }) => {
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    else await refresh(credentialId, currentPrefix, { soft: true });
  }
  ...
  async function onDelete(entry: R2Entry) {
    ...
    try {
      ...
      if (searchQuery) await runSearch(searchQuery);
      else await refresh(id, prefix, { soft: true });
    } catch (err) {
  ...
  async function onCalculateTotal() {
    ...
    try {
      const res = await api.prefixStats(id, prefix);
      ...
      if (reqId === statsRequestId.current) setStatsLoading(false);
    }
  }
  ...
  async function onDeleteCredential() {
    if (!id) return;
    if (!confirm(`Delete credential “${label || "this credential"}”? This does not delete files in R2.`)) {
      return;
    }
    try {
    ...
    >
      <header className="topbar">
        ...
        <div className="row wrap">
          <button className="ghost" onClick={() => void onRefresh()} disabled={!id || loading || refreshing || !!uploading}>
          ...
          <button className="ghost" onClick={() => void openEdit()} disabled={!id || credBusy === "load"}>
            Edit
          </button>
          <button className="ghost danger" onClick={() => void onDeleteCredential()} disabled={!id}>
            Delete
          </button>
          <input
            ref={fileInputRef}
      ...
      <nav className="crumbs">
        {crumbs.map((c, i) => (
          <button key={c.path || "root"} className="linkish" onClick={() => void goTo(c.path)}>
      ...
      {showEdit && (
        <form className="card form-grid" onSubmit={(e) => void onSaveCredential(e)}>
          <h2 style={{ gridColumn: "1 / -1", margin: "0 0 0.25rem", fontSize: "1.1rem" }}>
            {credBusy === "load" ? "Loading credential…" : "Edit credential"}
          </h2>
          {(["label", "accountId", "bucket", "endpoint", "accessKeyId", "secretAccessKey"] as const).map((key) => (
            <label key={key}>
              {key}
              <input
                ...
                value={credForm?.[key] ?? ""}
                onChange={(e) => onCredFormChange(key, e.target.value)}
              />
            </label>
          ))}
          <div className="row" style={{ gridColumn: "1 / -1", marginTop: 8 }}>
            <button type="button" className="ghost" disabled={credBusy !== null || !credForm} onClick={() => void onTestCredential()}>
              {credBusy === "test" ? "Testing…" : "Test connection"}
            </button>
            <button type="submit" disabled={credBusy !== null || !credForm || !credTestedOk}>
              {credBusy === "save" ? "Saving…" : credTestedOk ? "Update credential" : "Test connection first"}
            </button>
            <button type="button" className="ghost" onClick={() => { setShowEdit(false); setCredForm(null); }}>
              Cancel
            </button>
          </div>
        </form>
      ...
      {showNewFolder && (
        <form className="inline-form" onSubmit={(e) => void onCreateFolder(e)}>
          <input ref={folderInputRef} value={folderName} onChange={(e) => setFolderName(e.target.value)} placeholder="Folder name" required />
          <button type="submit" disabled={folderBusy}>{folderBusy ? "Creating…" : "Create"}</button>
          <button type="button" className="ghost" onClick={() => setShowNewFolder(false)}>Cancel</button>
        </form>
      )}

      {error && <div className="banner error">{error}</div>}
      {info && <div className="banner ok">{info}</div>}
      {uploading && <div className="banner ok">Uploading {uploading}… {progress}%</div>}

      <div className={`drop-hint ${dragOver ? "active" : ""}`}>
        {dragOver ? "Drop files to upload…" : "Drag files here to upload"}
      </div>

      {!loading && (
        <form className="search-bar" onSubmit={(e) => { e.preventDefault(); void runSearch(searchInput); }}>
          <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder={prefix ? "Search in this folder…" : "Search in this bucket…"} disabled={searching || !!uploading} />
          {isSearchMode && <button type="button" className="ghost" onClick={clearSearch}>Clear</button>}
          <button type="submit" disabled={searching || !searchInput.trim()}>{searching ? "Searching…" : "Search"}</button>
        </form>
      )}

      {isSearchMode && (
        <div className="search-meta muted">
          Results for “{searchQuery}”{searchTruncated ? " (partial)" : ""} · {searchResults.length} found
        </div>
      )}
      ...
      {!loading && (
        <div className="stats-bar">
          <div className="stats-metrics">
            <div className="stats-metric"><span className="stats-label">Files</span><strong className="stats-value">{listingStats.files}</strong></div>
            <div className="stats-metric"><span className="stats-label">Folders</span><strong className="stats-value">{listingStats.folders}</strong></div>
            <div className="stats-metric"><span className="stats-label">Visible size</span><strong className="stats-value">{listingStats.files > 0 ? formatSize(listingStats.bytes) : "—"}</strong></div>
            <div className="stats-metric stats-metric-wide">
              <span className="stats-label">{prefix ? "Folder total" : "Bucket total"}</span>
              <strong className="stats-value">
                {statsLoading ? "…" : deepStats ? `${formatSize(deepStats.totalBytes)}${deepStats.truncated ? "+" : ""} · ${deepStats.objectCount} obj` : "—"}
              </strong>
            </div>
          </div>
          <button type="button" className="ghost stats-action" disabled={statsLoading || !!uploading} onClick={() => void onCalculateTotal()}>
            {statsLoading ? "Calculating…" : deepStats ? "Recalculate" : "Calculate total"}
          </button>
        </div>
      )}
      ...
      {loading || searching ? (
        ...
        <div className="empty">{isSearchMode ? "No matching files" : "This folder is empty"}</div>
      ) : (
        <div className={`file-table-wrap ${refreshing ? "is-refreshing" : ""}`}>
            ...
            <tbody>
              {visibleEntries.map((entry) => (
                <tr key={entry.key}>
                  <td>
                    {entry.type === "folder" ? (
                      <button className="linkish" onClick={() => void (isSearchMode ? openSearchResult(entry) : goTo(entry.key.replace(/\/$/, "")))}>
                        📁 {entry.name}
                      </button>
                    ) : isSearchMode ? (
                      <button className="linkish" onClick={() => void openSearchResult(entry)}>📄 {entry.name}</button>
                    ) : (
                      <span>📄 {entry.name}</span>
                    )}
                  </td>
                  <td className="muted">{entry.type === "file" ? formatSize(entry.size) : "—"}</td>
                  <td className="muted">{entry.lastModified ? new Date(entry.lastModified).toLocaleString() : "—"}</td>
                  <td className="row end">
                    {entry.type === "file" && (
                      <button className="ghost" onClick={() => void onDownload(entry)}>Download</button>
                    )}
                    <button className="ghost danger" onClick={() => void onDelete(entry)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
