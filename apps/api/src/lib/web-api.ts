...
function friendlyNetworkError(err: unknown): Error {
  if (err instanceof TypeError) {
    return new Error("Cannot reach API. Check your internet connection.");
  }
...
export const api = {
  ...
  async uploadObject(
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      xhr.open("POST", `/api/credentials/${credentialId}/objects`);
      ...
      xhr.onerror = () =>
        reject(new Error("Upload failed — cannot reach API."));
      xhr.send(form);
    });
  },
